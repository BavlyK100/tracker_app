...
A personal habit-tracking web app built with **Flask** (Python). Create a cycle with your own start and end dates, add the goals you want to track, and check off each day as you go — with progress stats, streaks, and reflection built right in.
...
- **PDF export** — download any saved cycle as a PDF, with or without your reflection answers
...
- **Plain CSS** — no JavaScript anywhere, even the popups are done with a CSS-only trick
...
│   ├── dashboard.html       # Main page — active cycle + goals
│   ├── new_cycle.html      # Form to start a new cycle
│   ├── saved.html          # List of archived cycles
│   ├── cycle_detail.html   # View one archived cycle
...
Built by Bavly as a hands-on way to learn Flask, one feature at a time.
